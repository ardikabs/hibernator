...
| `chat_id` | Yes | Numeric chat ID or channel username (e.g., `@mychannel`) |
| `parse_mode` | No | `HTML` (default) or `MarkdownV2`. Controls how Telegram interprets formatting in the message. |

!!! tip "Auto-Escaping Reserved Characters"
    The Telegram Bot API requires certain characters to be escaped depending on the `parse_mode`. When `parse_mode` is `HTML`, symbols like `<`, `>`, and `&` must be replaced with HTML entities. When `parse_mode` is `MarkdownV2`, characters such as `_`, `*`, `[`, `]`, `(`, `)`, `~`, `` ` ``, `>`, `#`, `+`, `-`, `=`, `|`, `{`, `}`, `.`, `!` must be escaped with a preceding `\`.

    Hibernator handles this automatically via the `autoEscape` template function, which is injected into every Telegram render call. The built-in default template already pipes all dynamic values through `autoEscape`. If you write a [custom template](#custom-templates) for Telegram, use `{{ .SomeField | autoEscape }}` on any dynamic value to prevent parse errors.

    See the [Telegram Formatting Options](https://core.telegram.org/bots/api#formatting-options) for the full list of reserved characters per parse mode.

### Webhook

Delivers notifications as a JSON `POST` request to any HTTP endpoint. This is useful for integrating with custom alerting pipelines, incident management tools, or internal APIs.

**Secret schema:**

```json
{
  "url": "https://example.com/hooks/hibernator",
  "headers": {
...
| Field | Required | Description |
|-------|----------|-------------|
| `url` | Yes | The HTTP endpoint to POST notifications to |
| `headers` | No | Additional HTTP headers included in every request (e.g., auth tokens) |
| `enable_renderer` | No | When `true`, the payload is rendered through the template engine and included in the `rendered` field of the JSON body. Defaults to `false`. |

The webhook body is a JSON object with two fields:

| Field | Description |
|-------|-------------|
| `context` | The structured notification payload (event, plan, phase, targets, etc.) |
| `rendered` | The template-rendered message string. Only present when `enable_renderer` is `true`. |

!!! note "Custom Templates with Webhooks"
    If `enable_renderer` is `false` (the default), `templateRef` has no effect — the receiver gets the raw structured payload and can format it however it likes. Set `enable_renderer: true` when you want the controller to pre-render a human-readable message for the downstream system.
