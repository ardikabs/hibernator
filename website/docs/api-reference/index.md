...
| ----- | ---- | ----------- | ---------- |
| `serviceAccount` | _[ServiceAccountAuth](#serviceaccountauth)_ | ServiceAccount configures IRSA-based authentication. | [] |
| `static` | _[StaticAuth](#staticauth)_ | Static configures static credential-based authentication. | [] |
...
| `assumeRoleArn` | _string_ | AssumeRoleArn is the IAM role ARN to assume (optional).<br />Can be used with both ServiceAccount (IRSA) and Static authentication.<br />When using IRSA: the pod's SA credentials are used to assume this role.<br />When using Static: the static credentials are used to assume this role. | [] |
...
| `retries` | _integer_ | Retries is the maximum number of retry attempts for failed operations. | [Maximum: 10 Minimum: 0] |
...
| `aws` | _[AWSConfig](#awsconfig)_ | AWS holds AWS-specific configuration (required when Type=aws). | [] |
...
| `message` | _string_ | Message provides status details. | [] |
| `lastValidated` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | LastValidated is when credentials were last validated. | [] |
...
| `namespace` | _string_ | Namespace of the connector resource (defaults to plan namespace). | [] |
...
| `appliedAt` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | AppliedAt is when the exception was first applied. | [] |
...
| `shutdownExecution` | _[ExecutionOperationSummary](#executionoperationsummary)_ | ShutdownExecution summarizes the shutdown operation. | [] |
| `wakeupExecution` | _[ExecutionOperationSummary](#executionoperationsummary)_ | WakeupExecution summarizes the wakeup operation. | [] |
...
| `endTime` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | EndTime is when the operation completed. | [] |
| `targetResults` | _[TargetExecutionResult](#targetexecutionresult) array_ | TargetResults summarizes the result for each target. | [] |
| `success` | _boolean_ | Success indicates if all targets completed successfully. | [] |
| `errorMessage` | _string_ | ErrorMessage contains error details if the operation failed. | [] |
...
| `startedAt` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | StartedAt is when execution started. | [] |
| `finishedAt` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | FinishedAt is when execution finished. | [] |
| `attempts` | _integer_ | Attempts is the number of execution attempts. | [] |
| `message` | _string_ | Message provides human-readable status. | [] |
| `jobRef` | _string_ | JobRef is the namespace/name of the runner Job. | [] |
| `logsRef` | _string_ | LogsRef is the reference to logs (stream id or object path). | [] |
| `restoreRef` | _string_ | RestoreRef is the reference to restore metadata artifact. | [] |
| `serviceAccountRef` | _string_ | ServiceAccountRef is the namespace/name of ephemeral SA. | [] |
| `connectorSecretRef` | _string_ | ConnectorSecretRef is the namespace/name of connector secret. | [] |
| `restoreConfigMapRef` | _string_ | RestoreConfigMapRef is the namespace/name of restore hints ConfigMap. | [] |
...
| `maxConcurrency` | _integer_ | MaxConcurrency limits concurrent executions (for Parallel/DAG/Staged). | [Minimum: 1] |
| `dependencies` | _[Dependency](#dependency) array_ | Dependencies define DAG edges (only valid when Type=DAG). | [] |
| `stages` | _[Stage](#stage) array_ | Stages define execution groups (only valid when Type=Staged). | [] |
...
| `behavior` | _[Behavior](#behavior)_ | Behavior defines how failures are handled. | [] |
| `suspend` | _boolean_ | Suspend temporarily disables hibernation operations without deleting the plan.<br />When set to true, the plan transitions to Suspended phase and stops all execution.<br />When set to false, the plan transitions back to Active phase and resumes schedule evaluation.<br />Running jobs complete naturally but no new jobs are created while suspended. | [] |
...
| `lastTransitionTime` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | LastTransitionTime is when the phase last changed. | [] |
| `executions` | _[ExecutionStatus](#executionstatus) array_ | Executions is the per-target execution ledger. | [] |
| `observedGeneration` | _integer_ | ObservedGeneration is the last observed generation. | [] |
| `retryCount` | _integer_ | RetryCount tracks the number of retry attempts for error recovery. | [] |
| `lastRetryTime` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | LastRetryTime is when the last retry attempt was made. | [] |
| `errorMessage` | _string_ | ErrorMessage provides details about the error that caused PhaseError. | [] |
| `exceptionReferences` | _[ExceptionReference](#exceptionreference) array_ | ExceptionReferences is the history of schedule exceptions for this plan.<br />Maximum 10 entries, ordered by: active state first (most relevant), then by ValidFrom descending (most recent first).<br />Oldest entries are pruned when limit is exceeded. | [] |
| `currentStageIndex` | _integer_ | CurrentStageIndex tracks which stage is currently executing (0-based).<br />Reset to 0 when starting new hibernation/wakeup cycle. | [] |
| `currentOperation` | _string_ | CurrentOperation tracks the current operation type (shutdown or wakeup).<br />Used to determine which phase to transition to when stages complete. | [] |
| `executionHistory` | _[ExecutionCycle](#executioncycle) array_ | ExecutionHistory records historical execution cycles (max 5).<br />Each cycle contains shutdown and wakeup operation summaries.<br />Oldest cycles are pruned when limit is exceeded. | [] |
...
| Field | Type | Description | Validation |
| ----- | ---- | ----------- | ---------- |
| `kubeconfigRef` | _[KubeconfigRef](#kubeconfigref)_ | KubeconfigRef references a Secret containing kubeconfig. | [] |
| `inCluster` | _boolean_ | InCluster uses in-cluster config (for self-management). | [] |
...
| Field | Type | Description | Validation |
| ----- | ---- | ----------- | ---------- |
| `providerRef` | _[ProviderRef](#providerref)_ | ProviderRef references the CloudProvider (optional for generic k8s). | [] |
| `eks` | _[EKSConfig](#eksconfig)_ | EKS holds EKS-specific configuration. | [] |
| `gke` | _[GKEConfig](#gkeconfig)_ | GKE holds GKE-specific configuration. | [] |
| `k8s` | _[K8SAccessConfig](#k8saccessconfig)_ | K8S holds generic Kubernetes access configuration. | [] |
...
| `message` | _string_ | Message provides status details. | [] |
| `lastValidated` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | LastValidated is when connectivity was last validated. | [] |
| `clusterType` | _[K8SClusterType](#k8sclustertype)_ | ClusterType is the detected cluster type. | [Enum: [eks gke k8s]] |
...
| `daysOfWeek` | _string array_ | DaysOfWeek specifies which days this window applies to.<br />Valid values: MON, TUE, WED, THU, FRI, SAT, SUN | [MinItems: 1] |
...
| `namespace` | _string_ | Namespace is the namespace of the CloudProvider resource. | [] |
...
| `maxConcurrency` | _integer_ | MaxConcurrency limits parallelism within this stage. | [Minimum: 1] |
...
| `parameters` | _[Parameters](#parameters)_ | Parameters are executor-specific configuration. | [] |
...
| `executionId` | _string_ | ExecutionID is the unique identifier for this target execution. | [] |
| `startedAt` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | StartedAt is when execution started. | [] |
| `finishedAt` | _[Time](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.33/#time-v1-meta)_ | FinishedAt is when execution finished. | [] |
