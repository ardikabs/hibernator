...
func TestManager_SaveState_NoExisting(t *testing.T) {
	...
	// Verify CycleID is set at the data level for this target
	require.Equal(t, "cycle-1", loaded.CycleID, "CycleID should be set at data level")
	// Verify Status entries exist for both resources
	require.NotNil(t, loaded.Status, "Status map should be initialized")
	require.Contains(t, loaded.Status, "i-12345678", "Resource should have status entry")
	require.Contains(t, loaded.Status, "i-87654321", "Resource should have status entry")
}
...
func TestManager_SaveState_FirstSave_WithoutTrackingFields(t *testing.T) {
	...
	// Verify Status map is initialized with entries for all resources
	require.NotNil(t, loaded.Status, "Status map should be initialized")
	require.Contains(t, loaded.Status, "i-123", "Resource should have status entry")
	require.Contains(t, loaded.Status, "i-456", "Resource should have status entry")
	// Both should have StaleCount=0 since they were just reported
	require.Equal(t, 0, loaded.Status["i-123"].StaleCount, "Fresh resource should have StaleCount=0")
	require.Equal(t, 0, loaded.Status["i-456"].StaleCount, "Fresh resource should have StaleCount=0")
...
func TestManager_SaveState_BackwardCompatibility_NoTrackingFields(t *testing.T) {
	...
	require.NotEmpty(t, loaded.CycleID, "ManagedByCycleIDs should be initialized after merge")
	require.Equal(t, "cycle-2", loaded.CycleID)
	require.Equal(t, "cycle-2", loaded.CycleID)
}
...
func TestManager_SaveState_StalenessHousekeeping(t *testing.T) {
	...
	t.Run("staleness_accumulation_and_eviction", func(t *testing.T) {
		...
		require.NotEmpty(t, loaded.CycleID)
		require.Len(t, loaded.Status, 3, "All resources should have status entries")
		// All should have StaleCount=0 since they were just reported
		for _, key := range []string{"ng-active", "ng-stale", "ng-evict"} {
			require.Equal(t, 0, loaded.Status[key].StaleCount, "Fresh resource should have StaleCount=0")
		}
		...
		require.Equal(t, 1, loaded.Status["ng-stale"].StaleCount)
		require.Equal(t, 1, loaded.Status["ng-evict"].StaleCount)
		...
		require.Equal(t, "cycle-002", loaded.CycleID)
		// Note: CycleID is now at data level, not per-resource
		// Stale resources maintain their state but accumulate StaleCount
		...
		require.Equal(t, 2, loaded.Status["ng-stale"].StaleCount)
		require.Equal(t, 2, loaded.Status["ng-evict"].StaleCount)
		...
		require.Empty(t, loaded.Status["ng-stale"].StaleCount)
		require.Equal(t, "cycle-004", loaded.CycleID)
		// ng-evict reached threshold (3) and was EVICTED in this cycle
		require.Nil(t, loaded.State["ng-evict"])
		// Evicted resources don't have status entries
		require.NotContains(t, loaded.Status, "ng-evict", "Evicted resource should not have status entry")
	...
	t.Run("stale_resources_preserve_state_until_eviction", func(t *testing.T) {
		...
		require.Equal(t, 2, loaded.Status["i-123"].StaleCount)
		...
		require.Empty(t, loaded.Status["i-123"].StaleCount)
	})
	...
	t.Run("multiple_resources_partial_reporting", func(t *testing.T) {
		...
		require.Equal(t, 1, loaded.Status["ng-2"].StaleCount)
		require.Equal(t, 1, loaded.Status["ng-4"].StaleCount)
		// ng-1, ng-3, ng-5 should have no stale count and updated cycle
		require.Empty(t, loaded.Status["ng-1"].StaleCount)
		require.Empty(t, loaded.Status["ng-3"].StaleCount)
		require.Empty(t, loaded.Status["ng-5"].StaleCount)
		require.Equal(t, "cycle-002", loaded.CycleID)
		require.Equal(t, "cycle-002", loaded.CycleID)
		require.Equal(t, "cycle-002", loaded.CycleID)
	...
	t.Run("reported_but_not_in_demanded_state_drops_tracker", func(t *testing.T) {
		...
		require.Equal(t, "cycle-001", loaded.CycleID)
		...
		require.Empty(t, loaded.Status["i-123456"].StaleCount,
		...
		require.Equal(t, "cycle-003", loaded.CycleID,
...
func TestManager_SaveState_IdempotencyAndStaleness(t *testing.T) {
	...
	loaded, _ := mgr.Load(ctx, namespace, planName, targetName)
	require.Len(t, loaded.State, 3)
	// CycleID is set at data level for all resources in this target
	require.Equal(t, "cycle-001", loaded.CycleID)
	// All resources should have status entries with StaleCount=0
	require.Len(t, loaded.Status, 3, "All resources should have status entries")
	for _, key := range []string{"i-11111111", "i-22222222", "i-33333333"} {
		require.Equal(t, 0, loaded.Status[key].StaleCount, "Fresh resource should have StaleCount=0")
	}
	...
	require.NotEmpty(t, loaded.CycleID, "Same cycle ID restart: previously marked resource should preserve marker")
	...
	require.Len(t, loaded.Status, 3, "All resources should have status entries")
	for _, key := range []string{"i-11111111", "i-22222222", "i-33333333"} {
		require.Equal(t, 0, loaded.Status[key].StaleCount, "Reported resource should have StaleCount=0")
	}
	...
	require.Equal(t, 1, loaded.Status["i-33333333"].StaleCount)
	...
	require.Equal(t, 2, loaded.Status["i-33333333"].StaleCount)
	...
	require.Empty(t, loaded.Status["i-33333333"].StaleCount)
	...
	require.Equal(t, 2, loaded.Status["i-22222222"].StaleCount)
	...
	require.Empty(t, loaded.Status["i-22222222"].StaleCount)
	...
	require.Equal(t, "cycle-008", loaded.CycleID)
	require.Len(t, loaded.State, 3)
}
...
func TestManager_SaveState_ManagedByCycleIDEdgeCases(t *testing.T) {
	...
	t.Run("mixed_executor_types", func(t *testing.T) {
		...
		require.Equal(t, "cycle-001", loaded.CycleID)
		// Verify status entries exist for both resources
		require.Len(t, loaded.Status, 2, "All resources should have status entries")
		require.Contains(t, loaded.Status, "i-123")
		require.Contains(t, loaded.Status, "i-456")
	...
	t.Run("cycle_id_never_preserved", func(t *testing.T) {
		...
		loaded, _ := mgr.Load(ctx, namespace, planName, targetName)
		require.Equal(t, "old-cycle", loaded.CycleID)
		...
		require.Equal(t, "new-cycle", loaded.CycleID)
		require.NotEqual(t, "old-cycle", loaded.CycleID)
	})
	...
	t.Run("evicted_then_readded", func(t *testing.T) {
		...
		// CycleID is at data level and reflects the last cycle that saved data
		require.Equal(t, "cycle-004", loaded.CycleID, "CycleID should reflect last save cycle")
